...
import * as mysqlSchema from "../../drizzle/pg-schema";
import * as pgSchema from "../../drizzle/pg-schema";
import { nanoid } from "nanoid";

function S(): any { return getDriver() === "pg" ? pgSchema : mysqlSchema; }

// Convenience accessors for signage tables via dual-driver
const signageScreens = () => S().signageScreens;
const signageScreenGroups = () => S().signageScreenGroups;
const signageScreenGroupMembers = () => S().signageScreenGroupMembers;
const signageContent = () => S().signageContent;
const signagePlaylists = () => S().signagePlaylists;
const signagePlaylistItems = () => S().signagePlaylistItems;
const signageProvisioningTemplates = () => S().signageProvisioningTemplates;
const wayfindingBuildings = () => S().wayfindingBuildings;
const wayfindingCompanyAssignments = () => S().wayfindingCompanyAssignments;
const wayfindingCompanyPresence = () => S().wayfindingCompanyPresence;
const signageHeartbeats = () => S().signageHeartbeats;
const signageAuditLog = () => S().signageAuditLog;
const kitchenMenuItems = () => S().kitchenMenuItems;
const gymSchedules = () => S().gymSchedules;
const locations = () => S().locations;
const companies = () => S().companies;
const accessLog = () => S().accessLog;
const users = () => S().users;
...
export const signageScreensRouter = router({
  ...
  stats: protectedProcedure.input(z.object({ locationId: z.number().optional() }).optional()).query(async ({ input }) => {
    ...
    return {
      ...
      online: screens.filter((s: any) => s.status === "online").length,
      offline: screens.filter((s: any) => s.status === "offline").length,
      provisioning: screens.filter((s: any) => s.status === "provisioning").length,
      error: screens.filter((s: any) => s.status === "error").length,
      maintenance: screens.filter((s: any) => s.status === "maintenance").length,
    };
  }),
...
export const signageContentRouter = router({
  ...
  create: adminProcedure.input(z.object({
    ...
    contentType: z.enum([
      "image", "video", "pdf", "html", "html_slide", "url", "menu_card", "wayfinding",
...
export const signagePlaylistsRouter = router({
  ...
  create: adminProcedure.input(z.object({
    ...
    const db = await getDb();
    if (!db) throw new Error("DB unavailable");
    const [result] = await db.insert(signagePlaylists()).values(input).returning();
...
export const wayfindingRouter = router({
  ...
  companyAssignments: protectedProcedure.input(z.object({
    ...
    const companyMap = new Map<number, any>(allCompanies.map((c: any) => [c.id, c]));
    const buildingMap = new Map<number, any>(allBuildings.map((b: any) => [b.id, b]));
  ...
  companyPresenceToday: publicProcedure.input(z.object({
    ...
    const companyIds = presenceRecords.map((p: any) => p.companyId);
    ...
    return presenceRecords.map((p: any) => {
      const company = companyMap.get(p.companyId);
      const assignment = allAssignments.find((a: any) => a.companyId === p.companyId);
  ...
  syncPresenceFromAccessLog: protectedProcedure.input(z.object({
    ...
    // Get unique user IDs
    const userIdSet = new Set<number>(entries.filter((e: any) => e.userId).map((e: any) => e.userId!));
    ...
    allUsers.filter((u: any) => userIds.includes(u.id) && u.companyId).forEach((u: any) => companyIds.add(u.companyId!));
    ...
    for (const companyId of Array.from(companyIds)) {
      const assignment = assignments.find((a: any) => a.companyId === companyId);
      const existing = await db.select().from(wayfindingCompanyPresence()).where(and(
...
export const signageDisplayRouter = router({
  ...
  getWayfindingData: publicProcedure.input(z.object({
    ...
    const allCompanies = await db.select().from(companies());
    const companyMap = new Map<number, any>(allCompanies.map((c: any) => [c.id, c]));
    const buildingMap = new Map<number, any>(buildings.map((b: any) => [b.id, b]));

    const presentCompanies = presenceRecords.map((p: any) => {
      const company = companyMap.get(p.companyId);
      const assignment = allAssignments.find((a: any) => a.companyId === p.companyId);
      const building = p.buildingId ? buildingMap.get(p.buildingId) : null;
      return {
    ...
    const enrichedAssignments = allAssignments.map((a: any) => ({
      ...a,
      companyName: companyMap.get(a.companyId)?.name || "Unknown",
      companyLogo: companyMap.get(a.companyId)?.logoUrl,
