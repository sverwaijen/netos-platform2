...
// Also fix the parking_spots type constraint
await client.query(`ALTER TABLE parking_spots DROP CONSTRAINT IF EXISTS "parking_spots_type_check"`).catch(()=>{});
await client.query(`ALTER TABLE parking_spots ADD CONSTRAINT "parking_spots_type_check" CHECK (type IN ('standard','ev_charging','handicap','reserved','compact','motorcycle','vip'))`).catch(()=>{});
console.log("✓ Fixed parking_spots type constraint");

// Truncate all target tables (cascade to avoid FK issues)
const truncTables = [
  "parking_pricing", "parking_spots", "parking_zones",
  "products", "product_categories",
  "ticket_sla_policies", "canned_responses", "ops_agenda", "crm_triggers", "resource_categories"
];
for (const t of truncTables) {
...
for (const loc of locations) {
  ...
  await client.query(`INSERT INTO parking_zones ("locationId", name, slug, "totalSpots", type, "accessMethod", "isActive", "createdAt") VALUES ($1,$2,$3,$4,'indoor','anpr',true,NOW())`,
    [loc.id, `${loc.name} Parking`, slug, loc.spots]);
}
console.log("✓ 7 parking zones");
...
for (const zone of zones) {
  for (let i = 1; i <= zone.totalSpots; i++) {
    ...
    await client.query(`INSERT INTO parking_spots ("zoneId", "spotNumber", type, status, "isActive") VALUES ($1,$2,$3,'available',true)`,
      [zone.id, `${floor}${String(spotNum).padStart(2, "0")}`, type]);
    spotCount++;
  }
}
console.log(`✓ ${spotCount} parking spots`);

// ─── PARKING PRICING ───
for (const zone of zones) {
  await client.query(`INSERT INTO parking_pricing ("zoneId", name, "rateType", "priceEur", "priceCredits", "appliesToType", "freeMinutes", "maxDailyCapEur", "isActive", "createdAt") VALUES ($1,'Standard Rate','hourly',3.50,7.00,'all',15,15.00,true,NOW())`, [zone.id]);
  await client.query(`INSERT INTO parking_pricing ("zoneId", name, "rateType", "priceEur", "priceCredits", "appliesToType", "freeMinutes", "maxDailyCapEur", "isActive", "createdAt") VALUES ($1,'Member Monthly','monthly',99.00,198.00,'members',0,0.00,true,NOW())`, [zone.id]);
}
console.log("✓ 14 parking pricing rules");
...
for (let i = 0; i < cats.length; i++) {
  await client.query(`INSERT INTO product_categories (name, slug, "sortOrder", "isActive", "createdAt") VALUES ($1,$2,$3,true,NOW())`, [cats[i].name, cats[i].slug, i+1]);
}
...
const products = [
  ...
  { name: "Meeting Room Catering", desc: "Coffee, tea & snacks (per person)", eur: 12.50, credits: 25.00, cat: "Extras", sku: "EX-001" },
  { name: "Fruit Basket", desc: "Fresh seasonal fruit basket", eur: 15.00, credits: 30.00, cat: "Extras", sku: "EX-002" },
  { name: "Printing (B&W)", desc: "Per page black & white", eur: 0.15, credits: 0.30, cat: "Extras", sku: "EX-003" },
  { name: "Printing (Color)", desc: "Per page color", eur: 0.35, credits: 0.70, cat: "Extras", sku: "EX-004" },
];
for (let i = 0; i < products.length; i++) {
  const p = products[i];
  await client.query(`INSERT INTO products ("categoryId", name, description, "priceCredits", "priceEur", sku, "isActive", "sortOrder", "createdAt") VALUES ($1,$2,$3,$4,$5,$6,true,$7,NOW())`,
...
for (const sla of slas) {
  await client.query(`INSERT INTO ticket_sla_policies (name, priority, "firstResponseMinutes", "resolutionMinutes", "isActive", "createdAt") VALUES ($1,$2,$3,$4,true,NOW())`,
...
const canned = [
  ...
  { title: "Need More Info", cat: "general", body: "Could you provide more details about the issue?", shortcut: "/moreinfo" },
  ...
  { title: "Access Issue", cat: "access", body: "We've reset your access credentials. Please try again.", shortcut: "/access" },
  { title: "Billing Question", cat: "billing", body: "Thank you for your billing inquiry. We've reviewed your account.", shortcut: "/billing" },
  { title: "Escalated", cat: "general", body: "This has been escalated to our facilities team. Update within 24 hours.", shortcut: "/escalate" },
  { title: "WiFi Troubleshooting", cat: "tech", body: "Try: 1) Forget network and reconnect 2) Restart device 3) Try guest network.", shortcut: "/wifi" },
];
for (const c of canned) {
  await client.query(`INSERT INTO canned_responses (title, body, category, shortcut, "usageCount", "isActive", "createdAt") VALUES ($1,$2,$3,$4,0,true,NOW())`,
    [c.title, c.body, c.cat, c.shortcut]);
}
console.log("✓ 8 canned responses");

// ─── OPS AGENDA ───
const now = Date.now();
const DAY = 86400000;
const ops = [
  { locId: 1, title: "Weekly deep clean", type: "cleaning", priority: "normal", status: "scheduled", offset: 5*DAY },
  { locId: 1, title: "HVAC filter replacement", type: "maintenance", priority: "high", status: "scheduled", offset: 6*DAY },
  { locId: 2, title: "New member onboarding", type: "meeting", priority: "normal", status: "scheduled", offset: 1*DAY },
  ...
  { locId: 7, title: "Opening preparation", type: "maintenance", priority: "urgent", status: "in_progress", offset: 2*DAY },
];
for (const item of ops) {
  const st = now + item.offset;
  await client.query(`INSERT INTO ops_agenda ("locationId", title, type, priority, status, "startTime", "endTime", "createdAt") VALUES ($1,$2,$3,$4,$5,$6,$7,NOW())`,
    [item.locId, item.title, item.type, item.priority, item.status, st, st + 7200000]);
}
console.log("✓ 8 ops agenda items");

// ─── CRM TRIGGERS ───
const triggers = [
  { name: "New Lead Welcome Email", desc: "Send welcome email on new lead", eventType: "lead_created", cond: {}, actions: { type: "send_email", templateId: 1 } },
  { name: "Follow-up After Tour", desc: "Follow-up 24h after tour scheduled", eventType: "stage_changed", cond: { newStage: "tour_scheduled" }, actions: { type: "send_email", templateId: 2, delaySeconds: 86400 } },
  { name: "Re-engage Cold Leads", desc: "Re-engage leads inactive 30 days", eventType: "lead_inactive", cond: { inactiveDays: 30 }, actions: { type: "send_email", templateId: 3 } },
  { name: "Notify Owner on Won Deal", desc: "Notify owner when deal is won", eventType: "stage_changed", cond: { newStage: "won" }, actions: { type: "notify_owner" } },
  { name: "Lost Lead Survey", desc: "Send survey 1h after lead lost", eventType: "stage_changed", cond: { newStage: "lost" }, actions: { type: "send_email", templateId: 4, delaySeconds: 3600 } },
];
for (const t of triggers) {
  await client.query(`INSERT INTO crm_triggers (name, description, "isActive", "eventType", conditions, actions, "executionCount", "createdAt") VALUES ($1,$2,true,$3,$4,$5,0,NOW())`,
    [t.name, t.desc, t.eventType, JSON.stringify(t.cond), JSON.stringify(t.actions)]);
}
console.log("✓ 5 CRM triggers");
...
for (let i = 0; i < resCats.length; i++) {
  const rc = resCats[i];
  await client.query(`INSERT INTO resource_categories (name, slug, description, icon, "sortOrder", "isActive", "createdAt") VALUES ($1,$2,$3,$4,$5,true,NOW())`,
    [rc.name, rc.slug, rc.desc, rc.icon, i+1]);
}
console.log("✓ 5 resource categories");

// ─── FINAL COUNTS ───
const countTables = ["parking_zones","parking_spots","parking_pricing","product_categories","products","ticket_sla_policies","canned_responses","ops_agenda","crm_triggers","resource_categories"];
console.log("\n─── Final Counts ───");
