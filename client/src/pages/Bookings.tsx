  ...
  return (
    <div className="space-y-8 p-1">
      ...
      <div className="flex items-end justify-between">
        ...
        <Link href="/locations">
          <button className="flex items-center gap-2 px-5 py-3 bg-[#627653] text-white text-[10px] font-semibold tracking-[3px] uppercase hover:bg-[#4a5a3f] transition-all">
        ...
        {(["upcoming", "past", "all"] as Tab[]).map((t) => {
          ...
          return (
            <button
              key={t}
              onClick={() => setTab(t)}
              className={`px-6 py-3 text-[10px] font-semibold tracking-[3px] uppercase transition-all border-b-2 ${
      ...
      {filtered.length === 0 ? (
        ...
        <div className="space-y-0">
          {filtered.map((b: any) => {
            ...
            return (
              <div key={b.id} className={`flex items-center justify-between py-4 border-b border-white/[0.03] hover:bg-white/[0.02] transition-colors ${isCancelled ? "opacity-40" : ""}`}>
