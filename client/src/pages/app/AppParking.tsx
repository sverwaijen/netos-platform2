...
// Map zone types to Dutch labels
      ...
      <div>
        ...
        <div className="space-y-2">
          {zones.length === 0 ? (
            ...
            zones.map((zone: any) => {
              // Use availableSpots from demo data, or estimate from totalSpots
              const available = zone.availableSpots ?? Math.floor(zone.totalSpots * 0.55);
              const occupancyPercent = ((zone.totalSpots - available) / Math.max(zone.totalSpots, 1)) * 100;
                ...
                <div key={zone.id} className="bg-white/[0.03] rounded-xl p-4">
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-lg bg-[#C4B89E]/10 flex items-center justify-center flex-shrink-0">
                        <MapPin className="w-5 h-5 text-[#C4B89E]" />
                      </div>
                      <div>
                        <p className="text-white text-sm font-medium">{zone.name}</p>
                        <p className="text-white/30 text-xs">{zoneTypeLabel[zone.type] || "Open"}</p>
                      </div>
                    </div>
                  </div>
                  <div className="flex items-center gap-4 mt-2">
                    ...
                    <span className="text-white/40 text-xs flex-shrink-0">
                      {available}/{zone.totalSpots} vrij
                    </span>
                  </div>
                </div>
              );
              ...
              <div key={res.id} className="bg-white/[0.03] rounded-xl p-4 flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-white/30 text-xs">
                    {new Date(res.reservationDate || res.date).toLocaleDateString("nl-NL", { weekday: "short", day: "numeric", month: "short" })}
                  </p>
                </div>
